...
class TaxiPriceHelper {
  static double calculateTripCost(List<Carts> cartList, UserData userData) {
    ...
    double distanceOrHour = rentalType == 'hourly'
        ? userData.estimatedHours ?? 1
        : rentalType == 'day_wise'
            ? (userData.estimatedHours ?? 0) / 24
            : userData.distance ?? 1;
    if (cartList.isEmpty) {
      return tripCost;
    }
  ...
  static double calculateDiscountCost(List<Carts> cartList, UserData userData,
      {required bool calculateProviderDiscount, double? tripCost}) {
    ...
    for (Carts cart in cartList) {
      double p = 0;
      if (cart.provider!.discount != null &&
          calculateProviderDiscount &&
          tripCost != null &&
          cart.provider!.discount!.minPurchase! <= tripCost) {
      ...
      if (rentalType == 'hourly') {
        p = PriceConverter.calculation(
            cart.vehicle!.hourlyPrice! * distanceOrHour,
            discountPrice,
            discountType,
            cart.quantity!);
      } else if (rentalType == 'day_wise') {
        p = PriceConverter.calculation(
            cart.vehicle!.dayWisePrice! * distanceOrHour,
            discountPrice,
            discountType,
            cart.quantity!);
      } else {
        p = PriceConverter.calculation(
            cart.vehicle!.distancePrice! * distanceOrHour,
            discountPrice,
            discountType,
            cart.quantity!);
      }
    ...
    if (calculateProviderDiscount) {
      if (tripCost != null) {
        if (cartList[0].provider!.discount != null &&
            cartList[0].provider!.discount!.minPurchase! <= tripCost &&
      ...
      } else if (tripCost == null &&
          cartList[0].provider!.discount != null &&
          cartList[0].provider!.discount!.maxDiscount! < tripDiscountCost) {
        tripDiscountCost = cartList[0].provider!.discount!.maxDiscount!;
      }
    }
  ...
  static double calculateDistanceWiseDiscount(
      VehicleModel vehicle, double discount, String discountType) {
    ...
    distanceWiseDiscount = PriceConverter.calculation(
        vehicle.distancePrice!, discount0, discountType0, 1);
    ...
    if (vehicle.provider != null && vehicle.provider!.discount != null) {
      ...
      if (vehicle.provider!.discount!.maxDiscount != 0 &&
          vehicle.provider!.discount!.maxDiscount! < distanceWiseDiscount) {
  ...
  static double calculateHourlyDiscount(
      VehicleModel vehicle, double discount, String discountType) {
    ...
    hourlyDiscount = PriceConverter.calculation(
        vehicle.hourlyPrice!, discount0, discountType0, 1);
    ...
    if (vehicle.provider != null && vehicle.provider!.discount != null) {
      ...
      if (vehicle.provider!.discount!.maxDiscount != 0 &&
          vehicle.provider!.discount!.maxDiscount! < hourlyDiscount) {
  ...
  static double getDiscountPrice(double providerDiscountPrice,
      double productDiscountPrice, double totalPrice) {
    double discountPrice = 0;
  ...
  static double getExtraDiscountPrice(
      double providerDiscountPrice, double productDiscountPrice) {
