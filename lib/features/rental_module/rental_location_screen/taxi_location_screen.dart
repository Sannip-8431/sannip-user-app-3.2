...
class TaxiLocationScreen extends StatefulWidget {
  ...
  const TaxiLocationScreen(
      ...
      this.vehicle,
      this.mapController,
      this.fromSuggestionScreen = false,
      this.fromSuggestionMap = false});
...
class _TaxiLocationScreenState extends State<TaxiLocationScreen> {
      ...
      if (widget.toAddress == null &&
        ...
        Future.delayed(const Duration(milliseconds: 200), () async {
          Get.find<TaxiLocationController>().setFromToMarker(
            _mapController,
            from: LatLng(double.parse(widget.fromAddress!.latitude!),
                double.parse(widget.fromAddress!.longitude!)),
            to: LatLng(double.parse(widget.toAddress!.latitude!),
                double.parse(widget.toAddress!.longitude!)),
            /*userData: widget.userData, canUpdateProfile: widget.userData != null,*/ clickFromButton:
                false,
          );
        });
      } else {
        ...
        Future.delayed(const Duration(milliseconds: 200), () async {
          Get.find<TaxiLocationController>().setFromToMarker(
            ...
            canUpdateProfile: widget.userData != null,
  ...
  Widget build(BuildContext context) {
    return GetBuilder<TaxiLocationController>(
      ...
      return Scaffold(
        appBar: CustomAppBar(
            title: widget.fromSuggestionMap! && !_isPicked
                ? taxiLocationController.isFormSelected
                    ? 'set_pickup_location'.tr
                    : 'set_destination_location'.tr
                : 'location'.tr),
        body: Stack(children: [
          GoogleMap(
            ...
            onCameraIdle: () {
              ...
              try {
                if (!taxiLocationController.showFromToMarker) {
                  taxiLocationController.updatePosition(_cameraPosition?.target,
                      taxiLocationController.isFormSelected, _mapController);
            ...
            style: Get.isDarkMode
                ...
                : Get.find<ThemeController>().lightMapTaxi,
          ),
          ...
          !taxiLocationController.showFromToMarker
              ? Center(
                  child: Padding(
                  padding: const EdgeInsets.only(
                      bottom: Dimensions.pickMapIconSize * 0.65),
                  child: taxiLocationController.isCameraMoving
          ...
          Positioned(
            ...
            child: AbsorbPointer(
              ...
              child: Container(
                ...
                child: widget.fromSuggestionMap! && !_isPicked
                    ? oneLocationView(taxiLocationController)
                    : doubleLocationView(),
              ),
            ),
          ),
          ...
          Positioned(
            right: 16,
            bottom: GetPlatform.isIOS ? 120 : 100,
            child: Column(children: [
              CustomFloatingActionButton(
                ...
                onTap: () {
                  _currentZoomLevel++;
                  _mapController
                      ?.animateCamera(CameraUpdate.zoomTo(_currentZoomLevel));
                },
              ),
              CustomFloatingActionButton(
                icon: Icons.remove,
              ...
              widget.fromSuggestionMap! && !_isPicked
                  ? Container(
                      width: 50,
                      height: 50,
                      decoration: BoxDecoration(
                        ...
                        onPressed: () {
                          _checkPermission(() {
                            taxiLocationController
                                .getCurrentLocation(_mapController);
                          });
                        },
          ...
          Positioned(
            ...
            child: Container(
              ...
              child: SafeArea(
                child: CustomButton(
                  buttonText: !taxiLocationController.inZone
                      ? 'service_not_available_in_this_area'.tr
                      : widget.fromSuggestionMap! && !_isPicked
                          ? 'pick_address'.tr
                          : 'confirm'.tr,
                  isLoading: taxiLocationController.isButtonClick &&
                      taxiLocationController.isLoading,
                  onPressed: taxiLocationController.isLoading ||
                          !taxiLocationController.inZone
                      ? null
                      : () async {
                          ...
                          if (widget.fromSuggestionMap! &&
                              !_isPicked &&
                              taxiLocationController.fromAddress != null &&
                              taxiLocationController.toAddress != null) {
                            setState(() {
                              _isPicked = true;
                            });
                            Get.find<TaxiLocationController>().setFromToMarker(
                              _mapController,
                              from: LatLng(
                                  double.parse(taxiLocationController
                          ...
                          } else if ((taxiLocationController.fromAddress ==
                                  null ||
                              taxiLocationController.toAddress == null)) {
                            Get.back();
                          } else {
                            await taxiLocationController.setFromToMarker(
                              _mapController,
                              fromAddress: taxiLocationController.fromAddress!,
                              from: LatLng(
                                  double.parse(taxiLocationController
                                      .fromAddress!.latitude!),
                                  double.parse(taxiLocationController
                              ...
                              to: LatLng(
                                  ...
                                  double.parse(taxiLocationController
                                      .toAddress!.longitude!)),
                              userData: widget.userData,
                              canUpdateProfile: widget.toAddress != null &&
                                  widget.userData != null,
    ...
    return Padding(
      ...
      child: Row(children: [
        CustomIconLayout(
          height: 20,
          width: 20,
          icon: taxiLocationController.isFormSelected
              ? Icons.location_on_rounded
              : null,
          iconImage: !taxiLocationController.isFormSelected
              ? Images.navigationArrowIcon
              : null,
        ...
        Expanded(
          child: Padding(
            ...
            child: Text(
              taxiLocationController.isFormSelected
                  ? taxiLocationController
                          .formTextEditingController.text.isEmpty
                      ? 'enter_pickup'.tr
                      : taxiLocationController.formTextEditingController.text
                  : taxiLocationController.toTextEditingController.text.isEmpty
                      ? 'enter_destination'.tr
                      : taxiLocationController.toTextEditingController.text,
    ...
    return Row(
      children: [
        const Padding(
          ...
          child: Column(children: [
            CustomIconLayout(
                height: 20, width: 20, icon: Icons.location_on_rounded),
            CustomVerticalDottedLine(
              dotCount: 6,
            ),
            CustomIconLayout(
                height: 15, width: 15, iconImage: Images.navigationArrowIcon),
          ]),
        ),
        Expanded(
            child: Column(children: [
          RiderAddressInputField(
            ...
            userData: widget.userData,
            vehicle: widget.vehicle,
            fromSuggestionScreen:
                (widget.fromSuggestionMap! || widget.fromSuggestionScreen!),
          ),
