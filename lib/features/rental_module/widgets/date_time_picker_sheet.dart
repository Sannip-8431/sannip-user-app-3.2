...
class _DateTimePickerSheetState extends State<DateTimePickerSheet> {
  ...
  void initState() {
    ...
    selectTripDate = widget.userData != null
        ? DateConverter.dateTimeStringToDate(widget.userData!.pickupTime!)
        : Get.find<TaxiLocationController>().selectedTripDate;
    selectTripTime = widget.userData != null
        ? DateConverter.dateTimeStringToDate(widget.userData!.pickupTime!)
        : Get.find<TaxiLocationController>().selectedTripTime;
  }
          ...
          child: SfDateRangePicker(
              ...
              onSelectionChanged: (DateRangePickerSelectionChangedArgs args) {
                DateTime selectedDate =
                    DateConverter.dateTimeStringToDate(args.value.toString());
                setState(() {
                  ...
                  ? Row(children: [
                      TextButton(
                        onPressed: () {
                          Get.find<TaxiLocationController>()
                              .setTripTime(DateTime.now());
                          Get.find<TaxiLocationController>()
                              .setTripDate(DateTime.now());
                          Get.find<TaxiLocationController>()
                              .setFinalTripDateTime(DateTime.now());

                          Get.find<TaxiLocationController>()
                              .takeCurrentTime(true);
                          if (widget.fromCart) {
                            _updateUserData(Get.find<TaxiLocationController>()
                                .finalTripDateTime!);
                          } else {
                            Get.back();
                          }
                      ...
                      CustomButton(
                        ...
                        onPressed: canUpdate
                            ? () {
                                Get.find<TaxiLocationController>()
                                    .setTripDate(selectTripDate);
                                Get.find<TaxiLocationController>()
                                    .setTripTime(selectTripTime);
                                DateTime date =
                                    DateConverter.formattingTripDateTime(
                                        Get.find<TaxiLocationController>()
                                            .selectedTripTime!,
                                        Get.find<TaxiLocationController>()
                                            .selectedTripDate!);
                                ...
                                if (DateConverter.isAfterCurrentDateTime(
                                  ...
                                  Get.find<TaxiLocationController>()
                                      .takeCurrentTime(
                                          DateConverter.isSameDate(date));
                                  if (widget.fromCart &&
                                      widget.userData != null) {
                                    _updateUserData(date);
                                  } else {
                                    Get.find<TaxiLocationController>()
                                        .setFinalTripDateTime(date);
                                    Get.back();
                                  }
                                } else {
                                  showCustomSnackBar(
                                      'you_cannot_select_before_current_time'
                                          .tr,
                                      getXSnackBar: true);
                                }
                                date = date;
  ...
  void _updateUserData(DateTime date) {
    CartLocation pick = CartLocation(
        lat: widget.userData!.pickupLocation!.lat,
        lng: widget.userData!.pickupLocation!.lng,
        locationName: widget.userData!.pickupLocation!.locationName);
    CartLocation destination = CartLocation(
        lat: widget.userData!.destinationLocation!.lat,
        lng: widget.userData!.destinationLocation!.lng,
        locationName: widget.userData!.destinationLocation!.locationName);
    CarCart cart = CarCart(
      applyMethod: true,
      distance: widget.userData!.distance,
      destinationTime: widget.userData!.destinationTime,
      rentalType: widget.userData!.rentalType,
      estimatedHour: '${widget.userData!.estimatedHours ?? 0}',
      pickupLocation: pick,
      destinationLocation: destination,
      pickupTime: DateConverter.formatDate(date),
    );
    Get.find<TaxiCartController>()
        .updateUserData(cart: cart, userId: widget.userData!.id!)
        .then((success) {
      if (success) {
        Get.back();
