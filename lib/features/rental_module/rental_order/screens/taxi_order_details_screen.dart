...
class _TaxiOrderDetailsScreenState extends State<TaxiOrderDetailsScreen> {
  ...
  void initState() {
    ...
    if (widget.phone == null) {
      Get.find<TaxiOrderController>()
          .getTripDetails(widget.tripId, willUpdate: false);
    }

    if (widget.fromCheckout!) {
      Future.delayed(const Duration(microseconds: 600), () {
        Get.bottomSheet(const ConfirmBookingRequestBottomSheet(
            isScheduleBooking: false, isBooking: true));
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return GetBuilder<TaxiOrderController>(builder: (taxiOrderController) {
      ...
      bool tripCanceled =
          tripDetailsModel?.tripStatus == TripStatusEnum.canceled.name;
      double discount = 0;
      ...
      if (tripDetailsModel != null) {
        halfCompleted =
            tripDetailsModel.tripStatus == TripStatusEnum.completed.name &&
                tripDetailsModel.paymentStatus != 'paid';
              ...
              child: Scaffold(
                appBar: AppBar(
                  ...
                  title: Column(children: [
                    Text('${'trip'.tr} # ${widget.tripId}',
                        style: robotoMedium),
                    Text(
                      tripDetailsModel.tripStatus ==
                              TripStatusEnum.completed.name
                          ? tripDetailsModel.paymentStatus != 'paid'
                              ? 'trip_cost'.tr
                              : tripDetailsModel.tripStatus!.tr
                          : tripDetailsModel.tripStatus!.tr,
                ...
                body: RefreshIndicator(
                  onRefresh: () async {
                    await taxiOrderController.getTripDetails(widget.tripId,
                        willUpdate: false);
                  },
                    ...
                    Expanded(
                      ...
                      child: Column(
                          ...
                          children: [
                            ...
                            if (!halfCompleted &&
                                tripDetailsModel.vehicleIdentity != null &&
                                tripDetailsModel.vehicleIdentity!.isNotEmpty)
                              DeliverymanView(
                                  vehicleIdentity:
                                      tripDetailsModel.vehicleIdentity),
                            if (!halfCompleted)
                              TripCalculationView(
                                tripCost: tripCost,
                                discount: discount,
                                    ...
                                    child: Column(
                                        ...
                                        children: [
                                          ...
                                          ElevatedButton(
                                            onPressed: () {
                                              Get.bottomSheet(
                                                  BookingCancelBottomSheet(
                                                      tripId: tripDetailsModel
                                                          .id!));
                                            },
  ...
  Widget bottomView(
      TripDetailsModel tripDetailsModel, double total, bool taxInclude) {
    return Container(
      ...
      child: SafeArea(
        child: tripDetailsModel.tripStatus == TripStatusEnum.completed.name &&
                tripDetailsModel.paymentStatus != 'paid'
            ? Padding(
                padding: const EdgeInsets.symmetric(
                ...
                child: CustomButton(
                  ...
                  onPressed: () {
                    showModalBottomSheet(
                      ...
                      context: context,
                      builder: (BuildContext context) => TaxiPaymentBottomSheet(
                          id: tripDetailsModel.id!, totalPrice: total),
                    ).then((success) {
            ...
            : tripDetailsModel.tripStatus == TripStatusEnum.completed.name &&
                    tripDetailsModel.paymentStatus == 'paid' &&
                    _canReview(tripDetailsModel.vehicleIdentity ?? []) &&
                    tripDetailsModel.provider!.reviewSection! &&
                        ...
                        child: ElevatedButton(
                          onPressed: () {
                            if (tripDetailsModel.vehicleIdentity!.isNotEmpty) {
                              ...
                              showCustomSnackBar(
                                  'not_found_any_assigned_vehicle'.tr);
                            }
                          },
                          style: ElevatedButton.styleFrom(
