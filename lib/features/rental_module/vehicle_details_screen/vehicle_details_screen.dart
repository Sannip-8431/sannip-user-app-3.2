...
class _VehicleDetailsScreenState extends State<VehicleDetailsScreen> {
  ...
  void initState() {
    ...
    isExistInCartPosition =
        Get.find<TaxiCartController>().isExistInCart(widget.vehicleId!);
    if (isExistInCartPosition != -1) {
      cartQuantity =
          Get.find<TaxiCartController>().getCartQuantity(isExistInCartPosition);
    }
  }
    ...
    if (vehicle != null) {
      if (vehicle.avgRating != 0) {
        _categoryList.add(CategoryModel(
            icon: Images.taxiStarIcon,
            name: vehicle.avgRating!.toStringAsFixed(2)));
      }
      if (vehicle.type != null) {
        _categoryList.add(
            CategoryModel(icon: Images.taxiCarSideIcon, name: vehicle.type!));
      }
      if (vehicle.seatingCapacity != null) {
        _categoryList.add(CategoryModel(
            icon: Images.taxiSeatIcon,
            name: '${vehicle.seatingCapacity!} ${'seats'.tr}'));
      }
      if (vehicle.airCondition!) {
        _categoryList
            .add(CategoryModel(icon: Images.taxiACIcon, name: 'ac'.tr));
      } else {
        _categoryList
            .add(CategoryModel(icon: Images.taxiACIcon, name: 'non_ac'.tr));
      }
      if (vehicle.transmissionType != null) {
        _categoryList.add(CategoryModel(
            icon: Images.taxiAutomaticIcon, name: vehicle.transmissionType!));
      }
      if (vehicle.engineCapacity != null) {
        _categoryList.add(CategoryModel(
            icon: Images.taxiLiterIcon, name: '${vehicle.engineCapacity!}_L'));
      }
      if (vehicle.fuelType != null) {
        _categoryList.add(CategoryModel(
            icon: Images.taxiPetrolIcon, name: vehicle.fuelType!));
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle: true,
        actions: [
          RantCartWidget(callback: (value) {
            if (isExistInCartPosition != -1) {
              setState(() {
                cartQuantity = Get.find<TaxiCartController>()
                    .getCartQuantity(isExistInCartPosition);
              });
            }
          }),
      ...
      body: GetBuilder<TaxiHomeController>(builder: (taxiHomeController) {
        ...
        if (vehicle != null) {
          totalVehicles =
              vehicle.totalVehicles != 0 ? vehicle.totalVehicles! : 0;
          discount = vehicle.discountPrice ?? 0;
          discountType = vehicle.discountType ?? 'percent';

          distanceWiseDiscount = PriceConverter.calculation(
              vehicle.distancePrice!, discount, discountType, 1);
          hourlyDiscount = PriceConverter.calculation(
              vehicle.hourlyPrice!, discount, discountType, 1);
          dayWiseDiscount = PriceConverter.calculation(
              vehicle.dayWisePrice!, discount, discountType, 1);
        }
                    ...
                    child: Column(
                        ...
                        children: [
                          VehicleDetailsBanner(
                              taxiHomeController: taxiHomeController,
                              discount: discount,
                          ...
                          Padding(
                            ...
                            child: Column(
                                ...
                                children: [
                                  ...
                                  //Trip Type
                                  (vehicle.tripHourly! || vehicle.tripDistance!)
                                      ? Column(
                                          crossAxisAlignment:
                                              CrossAxisAlignment.start,
                                          children: [
                                                                ...
                                                                Text(
                                                                    ...
                                                                    style: robotoRegular.copyWith(
                                                                        ...
                                                                        color: Theme.of(context)
                                                                            .disabledColor)),
                                                                Text(
                                                                    ' (${vehicle.provider!.ratingCount ?? 0}+)',
                                                                    style: robotoRegular.copyWith(
                                                                        fontSize:
                                                                            Dimensions
                                                                                .fontSizeSmall,
                ...
                GetBuilder<TaxiCartController>(builder: (taxiCartController) {
                  return GetBuilder<TaxiLocationController>(
                    ...
                    int isExistInCartPosition =
                        taxiCartController.isExistInCart(vehicle.id);
                    int quantity = 1;
                    int cartId = 0;
                    double price = 0;
                    ...
                    estimatedDay = (taxiCartController
                                .carCartModel?.userData?.estimatedHours ??
                            0) /
                        24;

                    if (isExistInCartPosition != -1) {
                      quantity = taxiCartController
                          .getCartQuantity(isExistInCartPosition);
                      cartId =
                          taxiCartController.getCartId(isExistInCartPosition) ??
                              0;

                      double a = rentalType == AppConstants.hourly
                          ? (vehicle.hourlyPrice ?? 0)
                          : rentalType == AppConstants.dayWise
                              ? (vehicle.dayWisePrice ?? 0)
                              : (vehicle.distancePrice ?? 0);
                      double b = rentalType == AppConstants.hourly
                          ? taxiCartController
                              .carCartModel!.userData!.estimatedHours!
                          : rentalType == AppConstants.dayWise
                              ? estimatedDay
                              : taxiCartController
                                  .carCartModel!.userData!.distance!;
                      priceWithoutDiscount = (a * b) * cartQuantity;

                      discountPrice = _calculateDiscount(priceWithoutDiscount,
                          discount, discountType, vehicle);
                      ...
                      child: Column(children: [
                        if (isExistInCartPosition != -1)
                          Row(
                              ...
                              children: [
                                Text(
                                  '${rentalType == AppConstants.dayWise ? 'duration'.tr : 'estimated'.tr} (${rentalType == AppConstants.hourly ? '${taxiCartController.carCartModel?.userData?.estimatedHours}${'hr'.tr}' : rentalType == AppConstants.dayWise ? '${estimatedDay.toStringAsFixed(0)} ${'day'.tr}' : '${taxiCartController.carCartModel?.userData?.distance?.toStringAsFixed(3) ?? 0}${'km'.tr}'})',
                                  style: robotoRegular.copyWith(
                        ...
                        SafeArea(
                          ...
                          isExistInCartPosition != -1
                                      ...
                                      onTap: taxiCartController.isLoading
                                          ...
                                          : () {
                                              if (quantity > 1) {
                                                ...
                                                taxiCartController.setQuantity(
                                      ...
                                      onTap: taxiCartController.isLoading
                                          ...
                                          : () {
                                              if (totalVehicles >
                                                ...
                                                showCustomSnackBar(
                                                    '${'you_cant_add_more_than'.tr} $totalVehicles ${'quantities_of_this_vehicle'.tr}');
                                              }
                                            },
                                      isIncrement: true,
                                      color: taxiCartController.isLoading ||
                                              cartQuantity == totalVehicles
                              ...
                              : const SizedBox(),
                          SizedBox(
                              width: isExistInCartPosition != -1
                                  ? Dimensions.paddingSizeSmall
                                  : 0),
                          Expanded(
                              flex: 5,
                              child: CustomButton(
                                buttonText: isExistInCartPosition != -1
                                    ? 'update_in_cart'.tr
                                    : 'rent_this_vehicle'.tr,
  ...
  void _addToCart(TaxiCartController taxiCartController, VehicleModel vehicle) {
    if ((taxiCartController.cartList.isEmpty &&
        Get.find<TaxiLocationController>().toAddress == null)) {
      ...
      CartLocation pick = CartLocation(
          lat: taxiCartController.carCartModel!.userData!.pickupLocation!.lat,
          lng: taxiCartController.carCartModel!.userData!.pickupLocation!.lng,
          locationName: taxiCartController
              .carCartModel!.userData!.pickupLocation!.locationName);
      CartLocation destination = CartLocation(
          lat: taxiCartController
              .carCartModel!.userData!.destinationLocation!.lat,
          lng: taxiCartController
              .carCartModel!.userData!.destinationLocation!.lng,
          locationName: taxiCartController
              .carCartModel!.userData!.destinationLocation!.locationName);
      double? estimatedDay =
          (taxiCartController.carCartModel!.userData!.estimatedHours ?? 0) * 24;
      ...
      CarCart cart = CarCart(
        ...
        rentalType: taxiCartController.carCartModel!.userData!.rentalType,
        estimatedHour:
            taxiCartController.carCartModel!.userData!.rentalType == 'day_wise'
                ? estimatedDay.toStringAsFixed(0)
                : taxiCartController.carCartModel!.userData!.estimatedHours
                    .toString(),
        destinationTime:
            taxiCartController.carCartModel!.userData!.destinationTime,
        distance: taxiCartController.carCartModel!.userData!.distance,
      );
      ...
      double? estimatedDay = (double.tryParse(Get.find<TaxiLocationController>()
                  .estimateDayController
                  .text) ??
              0) *
          24;
      ...
      CarCart cart = CarCart(
        ...
        rentalType: Get.find<TaxiLocationController>().tripType,
        estimatedHour: Get.find<TaxiLocationController>().tripType == 'day_wise'
            ? estimatedDay.toStringAsFixed(0)
            : Get.find<TaxiLocationController>().estimateTimeController.text,
        destinationTime: Get.find<TaxiLocationController>().duration,
        distance: Get.find<TaxiLocationController>().distance,
      );
  ...
  double _calculateDiscount(double priceWithoutDiscount, double discount,
      String discountType, VehicleModel vehicle) {
    double discountPrice = 0;
    if (discountType == 'amount') {
      discountPrice = PriceConverter.calculation(
          priceWithoutDiscount, discount, discountType, cartQuantity);
    } else {
      double pPrice = PriceConverter.convertWithDiscount(
          priceWithoutDiscount, discount, discountType)!;
      discountPrice = priceWithoutDiscount - pPrice;
    }

    return discountPrice > priceWithoutDiscount
