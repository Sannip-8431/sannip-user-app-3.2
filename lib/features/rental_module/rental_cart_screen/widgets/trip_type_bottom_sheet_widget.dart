...
class TripTypeBottomSheetWidget extends StatefulWidget {
  ...
  @override
  State<TripTypeBottomSheetWidget> createState() =>
      _TripTypeBottomSheetWidgetState();
}
...
class _TripTypeBottomSheetWidgetState extends State<TripTypeBottomSheetWidget> {
  ...
  void initState() {
    ...
    estimateTimeController.text =
        '${widget.userData != null && widget.userData!.estimatedHours != null && widget.userData!.estimatedHours! > 0 ? widget.userData!.estimatedHours! : ''}';
    estimateDayController.text = widget.userData != null &&
            widget.userData!.estimatedHours != null &&
            widget.userData!.estimatedHours! > 0
        ? (widget.userData!.estimatedHours! / 24).toStringAsFixed(1)
        : '';

    if (widget.userData != null) {
      Get.find<TaxiCartController>().selectTripType(
          widget.userData?.rentalType ?? 'hourly',
          willUpdate: false);
    }
  }
        ...
        return Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                child: Row(
                    ...
                    children: [
                      TripTypeCard(
                        ...
                        distanceMinPrice: Get.find<SplashController>()
                                .configModel!
                      ...
                      TripTypeCard(
                        tripType: 'day_wise',
                        dailyMinPrice: Get.find<SplashController>()
                                .configModel!
            ...
            (widget.userData != null
                    ? taxiCartController.tripType == 'hourly'
                ...
                : (widget.userData != null
                        ? taxiCartController.tripType == 'day_wise'
                  ...
                  return Row(children: [
                    ...
                    Expanded(
                      child: CustomButton(
                        ...
                        onPressed: () async {
                          String tripType = widget.userData != null
                              ? taxiCartController.tripType
                              : taxiLocationController.tripType;

                          if (tripType == 'hourly' &&
                              (estimateTimeController.text.isEmpty ||
                                  double.parse(estimateTimeController.text) ==
                                      0)) {
                            showCustomSnackBar('please_enter_estimate_time'.tr,
                                getXSnackBar: true);
                          } else if (tripType == 'day_wise' &&
                              (estimateDayController.text.isEmpty ||
                                  double.parse(estimateDayController.text) ==
                                      0)) {
                            showCustomSnackBar('please_enter_estimate_days'.tr,
                                getXSnackBar: true);
                          } else {
                            bool isCartExistType =
                                await CartHelper.checkTypeInCart(
                                    taxiCartController.cartList, tripType);
                            double? estimatedDay =
                                (double.tryParse(estimateDayController.text) ??
                                        0) *
                                    24;

                            CarCart cart = CarCart(
                              applyMethod: true,
                              distance: widget.userData!.distance,
                              destinationTime: widget.userData!.destinationTime,
                              rentalType: tripType,
                              estimatedHour: tripType == 'hourly'
                                  ? estimateTimeController.text
                                  : tripType == 'day_wise'
                                      ? estimatedDay.toStringAsFixed(1)
                                      : '${widget.userData!.estimatedHours ?? 0}',
                            );
                            ...
                            if (isCartExistType) {
                              taxiCartController
                                  .updateUserData(
                                      cart: cart, userId: widget.userData!.id!)
                                  .then((success) async {
                                if (success) {
