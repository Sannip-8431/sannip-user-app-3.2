...
class TaxiCartRepository implements TaxiCartRepositoryInterface<CarCart> {
  ...
  Future<CarCartModel?> _addToCart(CarCart cart) async {
    ...
    Response response = await apiClient.postData(
        '${AppConstants.addToCarCartUri}${!AuthHelper.isLoggedIn() ? '?guest_id=${AuthHelper.getGuestId()}' : ''}',
        cart.toJson());
  ...
  Future<Response> tripBook(
    ...
    Response response = await apiClient.postData(
        '${AppConstants.tripBookingUri}${!AuthHelper.isLoggedIn() ? '?guest_id=${AuthHelper.getGuestId()}' : ''}',
        data);
    return response;
  }
  ...
  Future<Response> updateUserData(
    ...
    Response response = await apiClient.postData(
        '${AppConstants.tripUpdateUserDataUri}/$userId${!AuthHelper.isLoggedIn() ? '?guest_id=${AuthHelper.getGuestId()}' : ''}',
        cart.toJson());
    return response;
  }
  ...
  Future<bool> clearTaxiCart(
    ...
    Response response = await apiClient.deleteData(
        '${AppConstants.removeAllCarCartUri}${vehicleId != null ? '?vehicle_id=$vehicleId&quantity=$quantity&pickup_time=$pickupTime&rental_type=$rentalType' : ''}'
  ...
  Future delete(int? id) async {
    CarCartModel? cartModel;
    Response response = await apiClient.deleteData(
        '${AppConstants.removeCarCartUri}/$id${!AuthHelper.isLoggedIn() ? '?guest_id=${AuthHelper.getGuestId()}' : ''}');
    if (response.statusCode == 200) {
      cartModel = CarCartModel.fromJson(response.body);
    }
    return cartModel;
  ...
  Future<CarCartModel?> clearMultipleTaxiCart(List<int> ids) async {
    CarCartModel? cartModel;
    Response response = await apiClient.deleteData(
        '${AppConstants.removeMultipleCarCartUri}?cart_ids=$ids${!AuthHelper.isLoggedIn() ? '&guest_id=${AuthHelper.getGuestId()}' : ''}');
    if (response.statusCode == 200) {
      cartModel = CarCartModel.fromJson(response.body);
    }
    return cartModel;
  ...
  Future<CarCartModel?> _getCartList() async {
    ...
    Response response = await apiClient.getData(
        '${AppConstants.getCarCartListUri}${!AuthHelper.isLoggedIn() ? '?guest_id=${AuthHelper.getGuestId()}' : ''}');
    if (response.statusCode == 200) {
      cartModel = CarCartModel.fromJson(response.body);
    }
    return cartModel;
  ...
  Future update(Map<String, dynamic> body, int? id) async {
    CarCartModel? cartModel;
    Response response = await apiClient.postData(
        '${AppConstants.updateCarCartUri}${!AuthHelper.isLoggedIn() ? '?guest_id=${AuthHelper.getGuestId()}' : ''}',
        body);
    if (response.statusCode == 200) {
      cartModel = CarCartModel.fromJson(response.body);
    }
  ...
  Future<TaxiCouponModel?> applyTaxiCoupon(
    ...
    Response response = await apiClient.getData(
        '${AppConstants.taxiCouponApplyUri}?code=$couponCode&provider_id=$providerId');
    if (response.statusCode == 200) {
  ...
  Future<Response> getTripTax(
    ...
    Response response = await apiClient.postData(
        '${AppConstants.getTripTaxUri}${!AuthHelper.isLoggedIn() ? '?guest_id=${AuthHelper.getGuestId()}' : ''}',
        data);
    return response;
  }
}
